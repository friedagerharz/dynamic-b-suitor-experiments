    ...
    with open(input_file, 'r', encoding='utf-8') as infile, open(output_file, 'w', encoding='utf-8') as outfile:
        for line in infile:
            ...
            if line.startswith("(Dynamic) affected nodes per run:"):
                ...
                match = re.match(r"Start comparison for the dynamic ([\w]+)-matching: insert ([\d]+) edge into the graph located at ([^\s]+) with ([\d]+) nodes and ([\d]+) edges.", line)
                if match:
                ...
                outfile.write(f"\n")
            elif not (line.startswith("[WARN ]: Not adding edge") or line.startswith("Experiment: dynamic-b-suitor") or line.startswith("Output:") or line.startswith("Error Output:") or line.isspace()):
                outfile.write(line)
